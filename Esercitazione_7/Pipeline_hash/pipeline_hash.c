...
void* print_occ()
{     
      icl_hash_t *T = icl_hash_create(NBUCKETS, NULL, NULL);
      if(T == NULL)
            fprintf(stderr, "ERRORE: creando la tabella hash\n"); // vado avanti lo stesso

      int at_least_one_inserted = 0;
      while(1)
      {
            char *word = pop(Q2);
            if(word == EOS)
            ...
            if(T != NULL && icl_hash_find(T, word) == NULL)
            {
                  icl_hash_insert(T, word, (void *)word);
                  at_least_one_inserted = 1;
            }
            else
      ...
      char *key, *value;
      int i;
      if(T != NULL && at_least_one_inserted == 1)
      {
            icl_hash_foreach(T, i, entry, key, value)
                  printf("%s\n", value);
      
            icl_hash_destroy(T, NULL, free);
      }
      return NULL;
}
...
int main(int argc, char *argv[])
{
      ...
      safe_pthread_create(&third, NULL, &print_occ, NULL);
